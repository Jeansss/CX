Dado('que o consumer tenha avançado com um pedido até o checkout com produto {string} na loja {string}') do |product_type, country|
  @home_page = Home.new

  @product_type = product_type

  store = "https://#{MASSA['stores'][country]}"
  visit(store)
...
Quando('o consumer finalizar um pedido com o meio de entrega {string} e método de pagamento {string} em loja br') do |shipping_option, payment_method|
  ...
  unless shipping_option.eql?('digital')
    ...
    #entrega
    @checkout_start_page.shippings.each do |option|

      break option.click if option.text.eql?(shipping_option)
    end

  end
  if shipping_option.eql?('Correios - SEDEX') || shipping_option.eql?('Correios - PAC') || shipping_option.eql?('Envio Especial')
    #dados destinatário
...
Então('validar que o pedido foi finalizado com sucesso') do |table|
  ...
  unless store.eql?('mx')
    expect(@checkout_success_page.prices.first.text.gsub(',','.')).to eql(@price.gsub(',','.'))
  else
    expect(@checkout_success_page.prices.first.text.gsub(',','.').gsub(/ [A-Z]+/, '')).to eql(@price.gsub(',','.'))
  end
...
E('o consumer finalizar um pedido com o meio de entrega {string} e método de pagamento {string} em loja ar') do |shipping_option, payment_method|
  ...
  unless shipping_option.eql?('digital')
    ...
    @checkout_start_page.btnNext.click
    #entrega
    @checkout_start_page.shippingOption.first.click if shipping_option.eql?('OCA Estándar - Envío a domicilio')
    @checkout_start_page.pickup.click if shipping_option.eql?('Retiras en EstoqueJean')
  ...
  if shipping_option.eql?('OCA Estándar - Envío a domicilio')
    #dados destinatário
    @checkout_start_page.name.set('test')
    @checkout_start_page.lastName.set('last')
...
E('o consumer finalizar um pedido com o meio de entrega {string} e método de pagamento {string} em loja mx') do |shipping_option, payment_method|
  @document = '1234567' if @document.nil?
  ...
  unless shipping_option.eql?('digital')
    ...
    @checkout_start_page.btnNext.click
    #entrega
    @checkout_start_page.shippingOption.first.click if shipping_option.eql?('Envio Especial')
    @checkout_start_page.pickup.click if shipping_option.eql?('Pedido para ser recogido en EstoqueJean')
  end
  if shipping_option.eql?('Envio Especial')
    #dados destinatário
    @checkout_start_page.name.set('test')
    @checkout_start_page.lastName.set('last')
    @checkout_start_page.tel.set('1199999999')
...
Dado('o consumer efetuar o login no checkout') do
  ...
  @account = Account.new
  @account.login.set('jeastestautomationsmoke@smoke.com')
...
Dado('o consumer aplica o cupom de desconto {string}') do |string|
  ...
  expect(@checkout_start_page.subtotal.text.gsub(/[A-Z]\S/, '').gsub('.','').to_f / 100 * 10).to eql(@checkout_start_page.desconto.text[4..@checkout_start_page.desconto.text.size].to_f)
  @valor_do_produto_com_desconto = @checkout_start_page.subtotal.text.gsub(/[A-Z]\S/, '').gsub('.','').to_f - @checkout_start_page.desconto.text[4..@checkout_start_page.desconto.text.size].to_f
